...
Rules = namedtuple('Rules',
                   ['protocol_id', 'src_ip', 'dst_ip', 'proto', 'src_port', 'dst_port', 'bandwidth', 'queue_id',
                    'uuid_queue'])
...
class ofp_flow_mod(ofp_header):
    ...
    def check_rules(self):
        ...
        if connect_db.connection_failed == False:
            ...
            protocol_number = {'tcp': "6", 'TCP': "6", 'udp': "17", 'UDP': "17", '17': "17", '6': "6", 'icmp': "1",
                               'ICMP': "1", '1': "1", 'any': "any"}
            ...
            for record_qos_setting in table_qos_setting:
                ...
                if record_qos_setting[1] != self.match.nw_src and record_qos_setting[1] != "any":
                    ...
                    if record_qos_setting[2] != self.match.nw_dst and record_qos_setting[2] != "any":
                        ...
                        if record_qos_setting[2] != "any":
                            count_any += 1
                        if protocol_number[record_qos_setting[5]] != str(self.match.nw_proto) and protocol_number[
                            record_qos_setting[5]] != "any":
                            flag = False
                        else:
                ...
                if flag == True:
                    temp_rule = Rules(record_qos_setting[0], record_qos_setting[1], record_qos_setting[2],
                                      record_qos_setting[5], record_qos_setting[3], record_qos_setting[4],
                                      record_qos_setting[6], None, None)
    ...
    def pack(self):
        ...
        packet += struct.pack('!QHHHHLHH', self.cookie, self.command, self.idle_timeout, self.hard_timeout,
                              self.priority, self.buffer_id, self.out_port, self.flags)
    ...
    def update_list_rule_added(self, r):
        for index, rule in enumerate(self.list_rule_added):
            if rule.protocol_id == r.protocol_id:
                temp = Rules(rule.protocol_id, r.src_ip, r.dst_ip, r.proto, r.src_port, r.dst_port, r.bandwidth,
                             rule.queue_id, rule.uuid_queue)
                ...
                if sock == sockCtrl:
                    ...
                    if len(data_from_controller) >= 8:
                        ...
                        if of_header.type == 14 and len(data_from_controller) >= 80:
                            logging.info('Detect FlowMod Package From Controller:%s to %s:%s', str(IP_CONTROLLER), str(self.ip), str(self.port))
                            # print "flowmod"
                            flowmod_msg = ofp_flow_mod(data_from_controller)
                            flowmod_msg.unpack()

                            logging.info('[%s:%s] Unpack FlowMod Success', str(self.ip), str(self.port))

                            flag1, rule = flowmod_msg.check_rules()
                            if flag1:
                                logging.info('[%s:%s] Hit in Rule Table', str(self.ip), str(self.port))
                                flag2, queue_id = self.isAdded(rule)
                            else:
                                logging.info('[%s:%s] Miss in Rule Table', str(self.ip), str(self.port))
                            ...
                            # print 'flag: ' + str(flag)
                            logging.info('[%s:%s] Connecting DB ....', str(self.ip), str(self.port))
                            update_db = connectDB.get_QoS_setting()
                            logging.info('[%s:%s] Connection Success', str(self.ip), str(self.port))
                            # print "flowmod_msg.actions: ", flowmod_msg.actions
                            # print "flowmod_msg.actions[0].out_port: "  + str(flowmod_msg.actions[0].out_port)
                            # print "nameport: " + str(name_port)

                            if flag1 and name_port is not None and (
                                                flowmod_msg.match.nw_proto == 6 or flowmod_msg.match.nw_proto == 17 or flowmod_msg.match.nw_proto == 1):
                                print "flowmod_msg.status_id: " + str(flowmod_msg.status_id)

                                if not flag2 and flowmod_msg.status_id == 0:

                                    self.current_id, queue_id, uuid_queue = callovsdb.create_queue(name_port,
                                                                                                   rule.bandwidth,
                                                                                                   self.current_id,
                                                                                                   str(self.ip), 6640)
                                    logging.info('[%s:%s] create queue success: ' + str(datetime.datetime.now().date()) + ' ' + str(
                                        datetime.datetime.now().time()) + ' queue#' + str(queue_id) + ' in ' + str(
                                        name_port), str(self.ip), str(self.port))
                                    r = Rules(rule.protocol_id, rule.src_ip, rule.dst_ip, rule.proto, rule.src_port,
                                              rule.dst_port, rule.bandwidth, queue_id, uuid_queue)
                                    ...
                                    #logging.info(str(datetime.datetime.now().date()) + ' ' + str(
                                    #    datetime.datetime.now().time()) + ' ' + str(r))
                                    ...
                                    if flag_on_process:
                                        ...
                                        callovsdb.update_queue(rule_update.uuid_queue, rule_update.bandwidth,
                                                               rule_update.bandwidth, 1, str(self.ip), 6640)
                                        ...
                                        self.current_id, queue_id, uuid_queue = callovsdb.create_queue(name_port,
                                                                                                       ...
                                                                                                       str(self.ip),
                                                                                                       6640)
                                        logging.info('[%s:%s] create queue: ' + ' queue#' + str(queue_id) + ' in ' + str(name_port), str(self.ip), str(self.port))

                                        r = Rules(rule.protocol_id, flowmod_msg.match.nw_src, flowmod_msg.match.nw_dst,
                                                  flowmod_msg.match.nw_proto, flowmod_msg.match.tp_src,
                                                  flowmod_msg.match.tp_dst, rule.bandwidth, queue_id, uuid_queue)
                                        #logging.info(str(datetime.datetime.now().date()) + ' ' + str(
                                        #   datetime.datetime.now().time()) + ' ' + str(r))
                                        # self.list_rule_added.append(r)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename='/home/wisarutk/FGBAM/fgbam.log', level=logging.DEBUG, format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S.%f %p')
    ...
    try:
        while True:
            ...
            logging.info('Device %s:%s connected', addr[0], addr[1])
